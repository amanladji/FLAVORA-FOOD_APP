...
<div>
	<form action="<%= request.getContextPath()%>/callMenuManager_CRUD" method="post" enctype="multipart/form-data"> 
		...
		<div>
			<label>ItemName</label><input type="text" name="itemName" value="<%=menu!= null ? menu.getItemName():"" %>" required>
		</div>
		<div>
			<label>Description</label><textarea rows="3" name="description" required><%=menu!= null ? menu.getDescription():"" %></textarea>
		</div>
		<div>
			<label>Price</label><input type="number" name="price" value="<%=menu!= null ? menu.getPrice():"" %>"required>
		</div>
		<div>
		...
		<div>
			<label>Category</label><input type="text" name="category" value="<%=menu!= null ? menu.getCategory():"" %>"required>
		</div>
		
		<input type="hidden" name="oldImage" value="<%=menu!= null ? menu.getImages():"" %>">
		<%if(menu != null){ %>
			<div>
				<img alt="image could not load" src="<%= request.getContextPath()%>/assets/<%=menu.getImages()%>" width="150" height="100">
			</div>
		<%} %>
		<br><br>
		<div>
...
%>
	<div class="card">
		<img alt="image not load" src="<%=request.getContextPath() %>/assets/<%=m.getImages() %>" class="menu-img">
		<h2><%=m.getItemName() %></h2>
		...
		<form action="<%= request.getContextPath()%>/callMenuManager_CRUD" method="post">
